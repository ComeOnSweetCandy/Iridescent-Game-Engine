...
class __IE_DLL__ IETerrainGrid :public IEBlock
{
public:
	IETerrainGrid();
	virtual ~IETerrainGrid();
	virtual void Initialization(unsigned int terrainID, IETerrainMode terrainMODE, unsigned int createdOrder);
	static IETerrainGrid * Create(unsigned int terrainID, IETerrainMode terrainMODE, unsigned int createdOrder);
...
public:
	...
	void AddNewCalBorder(IETerrainGrid ** grids);
	void DelOldCalBorder(IETerrainGrid ** grids);
